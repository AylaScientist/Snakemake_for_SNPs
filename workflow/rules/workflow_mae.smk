...
rule valids_mae:
    input:
        result1="results/SNPs_ready_mae.csv",
    output:
        result4="results/SNPs_ready_mae_valid_genome.csv",
        error = "results/Error_in_SNP_calling_mae_valid_genome.csv"
    log:
        "logs/python/ASE_data_wrangling/valids.log"
    conda:
        "envs/python.yaml"
...
rule QC_mae:
    input:
        i1="results/SNPs_ready_mae_valid_genome.csv",
        i2="config/Experimental_groups.csv",
        sn1="config/Sample_names.csv"
    output:
        results="results/QC_mae_valid_genome.csv"
    conda:
        "envs/python.yaml"
    resources:
        mem_mb=200000
