...
trimmer = " ".join(snakemake.params.get("trimmer"))

# Distribute threads
input_files = [snakemake.input.get("r1"), snakemake.input.get("r2")]
output_files = [
    snakemake.output.get("r1"),
    snakemake.output.get("r1_unpaired"),
    snakemake.output.get("r2"),
    snakemake.output.get("r2_unpaired"),
]

trimmomatic_threads, input_threads, output_threads = distribute_threads(
    input_files, output_files, snakemake.threads.[0]
)

input_r1, input_r2 = [
    compose_input_gz(filename, input_threads)
...
shell(
    "java -Xmx1g "
    "-jar /data/bin/miniconda2/envs/trimmomatic-v038/share/trimmomatic-0.38-1/trimmomatic.jar PE "
    "-XX:ParallelGCThreads={trimmomatic_threads} "
    "{extra} "
    "{input_r1} {input_r2} "
